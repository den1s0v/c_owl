...
class JsonNode(object):
	...
	def get_children(self):
		...
		for k in vars(self):   # ! переопределение стандартных методов типа __dict__ ломает vars(self)
			if k in ('parent', 'owner'):  # !!! ходим только по дочерним данным, игнорируя все где-либо определёные указатели на родительские узлы!
				continue
			val = getattr(self, k)
	...
	def search_up(self, node_class_or_func=None):
		...
		if is_me:  return self
		elif self.parent:  return self.parent.search_up(node_class_or_func)
		else:  print('!!! %s could not be found in the tree!' % node_class_or_func.__name__); return None
...
class ElseIfBranchJN(ConditionalAlternativeBranch):
	def __init__(self, jn_type="unkn", then=None, **kw):
		super(ElseIfBranchJN, self).__init__(jn_type, cond=kw["else if"], stmt=then, **{k:kw[k] for k in kw if k!="else if"})
...
class WhileJN(ConditionalLoop):
	def __init__(self, jn_type="unkn", name="noname", body=None, **kw):
		super(WhileJN, self).__init__(jn_type, name, cond=kw["while"], body=body, **{k:kw[k] for k in kw if k!="while"})
		...
		if node.body:
			...
			while 1:
				...
				if self.last_cond_tuple[1] == False:
					break
				# self.loop_make_iteration(node)
				act = IterationActLine(node.name, loop_act_id=node.ID, n=iter_n, executes=node.ID)  # для body нет отдельного узла!
				self.add_act(CompoundActNode(act), enter=True)
				self.visit_list(node.body)
				self.leave_current_act()
				iter_n += 1
...
class DoWhileJN(ConditionalLoop):
	def __init__(self, jn_type="unkn", name="noname", body=None, **kw):
		super(DoWhileJN, self).__init__(jn_type, name, cond=kw["do while"], body=body, **{k:kw[k] for k in kw if k!="do while"})
...
class ForJN(ConditionalLoop):
	def __init__(self, jn_type="unkn", name="noname", body=None, **kw):
		...
		assert not ("from" in kw and "init" in kw), "`for by {var}` got `from` and `init` keys both. Pass one of that instead.".format(var=self.loop_var)
		...
		try:
			...
			raise Exception("`for by {var}` got invalid value for `step`: '{val}'".format(var=self.loop_var, val=self.step))
		self.update_stmt = "{v}={v}{:+d}".format(self.step, v=self.loop_var)

		assert not ("to" in kw and "while" in kw), "`for by {var}` got `to` and `while` keys both. Pass one of that instead.".format(var=self.loop_var)
		...
		super(ForJN, self).__init__(jn_type, name,
			cond=self.cond_act,
			body=body, **{k:kw[k] for k in kw if k not in {"for", "from", "init", "to", "while", "step", "init_act", "cond_act", "update_act"}})
...
if AlgTextVisitor:
	def _(visitor, node):
		""" Вывести метку начала цикла, а имя и тело - вызвав базовый класс, в конце - заголовок. """
		visitor.add_line(tr(node.jn_type)+" ").visit(node.loop_var, same_line=True).append_at_line(" "+tr("from") + " ")
...
if TraceTextVisitor:  # visit
	...
	def _(self, node):  # make_iteration
		...
		self.add_line("%s %s (" % (tr("(he) performed"), tr("update"))).visit(node.update_act, same_line=True).append_at_line(")").append_nth_str(upd_i)
...
class ActLine(GlobalCounter, WithID):
	def __init__(self, action, executes="-NA-", n="auto", gen="it", phase="performed", comment="", appearance="normal", **kw):
...
class IterationActLine(ActLine):
	...
	def __init__(self, loop_name, loop_act_id="-no-loop-act-id-", appearance="reversed", **kw):  # executes="-a-loop-body-", n="auto", comment=""):
...
class CtrlStructActLine(ActLine):
	def __init__(self, struct_type=None, struct_name="-unnamed-ctrl-struct-", appearance="reversed", **kw):  # executes="-a-ctrl-struct-", n="auto", comment=""
...
class MultiWordActLine(ActLine):
	def __init__(self, action_words, value=None, appearance="normal", **kw):  # executes="-a-ctrl-struct-", n="auto", comment=""
