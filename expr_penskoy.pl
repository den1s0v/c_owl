...
% Rule: rule_next_prev []
swrl_rule() :- 
	index(A, ^^(A_INDEX,_)), add(B_INDEX, A_INDEX, 1), index(B, ^^(B_INDEX,_)), same_step(A, B),
...
% Rule: rule_next_step []
swrl_rule() :- 
	index(A, ^^(A_INDEX,_)), index(B, ^^(A_INDEX,_)), step(A, ^^(A_STEP,_)), add(B_STEP, A_STEP, 1), step(B, ^^(B_STEP,_)),
